...
```
Dans ce contexte, la meilleure combinaison de lois en termes d'estimation GMH *des trois paramètres* semble donc être soit $\alpha \sim \mathcal{L\textit{og-}N} \text{ et } \beta \sim \mathcal{L\textit{og-}N}$, soit $\alpha \sim \mathcal{L\textit{og-}N} \text{ et } \beta \sim \mathcal{U}$. En effet, lorsque $\alpha \sim \mathcal{G}$, l'algorithme tend à sous-évaluer $\beta$ et sur-évaluer $\alpha$. Visuellement, nous constatons aussi que GMH estime généralement des valeurs plus élevées de $X_t$ que celles du processus original, sauf dans le cas $\alpha \sim \mathcal{L\textit{og-}N} \text{ et } \beta \sim \mathcal{U}$. En associant ces deux arguments, nous pouvons donc conclure que la meilleure combinaison de lois est $\alpha \sim \mathcal{L\textit{og-}N} \text{ et } \beta \sim \mathcal{U}$.
Suivant le choix des lois, GMH peut être meilleur ou moins bon que MLE : le choix de l'un ou l'autre dépend donc de la justesse des lois a priori choisies **(\@ref(fig:InfConc))**.
...
ggarrange(plot1, plot2, plot3, plot4, common.legend = TRUE)
```
Encore une fois, les meilleurs combinaisons de lois en termes d'estimation GMH *des trois paramètres* semble être soit $\alpha \sim \mathcal{L\textit{og-}N} \text{ et } \beta \sim \mathcal{L\textit{og-}N}$, soit $\alpha \sim \mathcal{L\textit{og-}N} \text{ et } \beta \sim \mathcal{U}$. Nous constatons aussi que MLE sous-estime $\theta$. Ainsi, dans ce cas là, le meilleur choix d'estimations semble être le bayésien, avec une des deux combinaisons citées **(\@ref(fig:InfLin))**.
...
ggarrange(plot1, plot2, plot3, plot4, common.legend = TRUE)
```
Visuellement, nous constatons que MLE semble être très bon pour approcher le processus, les deux courbes étant confondues. GMH semble plutôt le surestimer. Toutefois, en regardant de plus près les estimations de chaque paramètre, nous remarquons que seule l'estimation de $\beta$ par MLE est bonne. Ainsi, aucune des deux estimations ne semble bien  adaptée Le choix portera donc plutôt sur l'objectif final d'estimation, i.e. retrouver les vrais paramètres ou retrouver la forme du processus **(\@ref(fig:InfConv))**.
\
...
ggarrange(plot1, plot2, plot3, plot4, common.legend = TRUE)
```
Peu importe la combinaison de lois choisies, GMH est toujours mauvais pour estimer les paramètres, et il se trompe toujours de la même manière : il sur-estime $\theta$ et sous-estime $\alpha$. Cette erreur peut nous faire penser au rôle similaire que jouent ces deux paramètres sur la forme de la courbe, i.e. de coefficient de proportionnalité. À l'inverse, MLE donne une bonne estimation de chaque paramètre. Par contre, les deux courbes estimées reste toujours assez proches du vrai processus. Il semble donc préférable d'utiliser MLE ici **(\@ref(fig:Alpha1))**.


### $\alpha = 10$
Nous regardons ensuite le cas $\alpha=10$, toujours avec les même conditions de départ pour les autres paramètres.
...
ggarrange(plot1, plot2, plot3, plot4, common.legend = TRUE)
```
Encore une fois, GMH sous-estime toujours $\alpha$ et sur-estime $\theta$. MLE reste encore assez bon, bien que nous puissions constater une dégradation de ses estimations (elles sont moins proches des vraies valeurs que précédemment). Les deux courbes estimées reste encore une fois toujours assez proches du vrai processus **(\@ref(fig:Alpha10))**.


### $\alpha = 100$
Nous regardons finalement $\alpha=100$, toujours avec les même conditions de départ pour les autres paramètres.
...
ggarrange(plot1, plot2, plot3, plot4, common.legend = TRUE)
```
Les observations suivent la ligne de conduite observée précédemment : GMH reste mauvais pour estimer $\alpha \text{ et } \theta$ les estimations de MLE continue de se dégrader et les courbes estimées reste assez proches du vrai processus **(\@ref(fig:Alpha100))**.
\
...
### $\theta = 10$
Nous regardons maintenant l'impact pour $\theta = 10$, toujours avec les même conditions de départ pour les autres paramètres.
...
### $\theta = 100$
Nous regardons finalement l'impact pour $\theta = 100$, toujours avec les même conditions de départ pour les autres paramètres.
...
ggarrange(plot1, plot2, plot3, plot4, common.legend = TRUE)
```
nous obtenons toujours le même constat qu'avec $\theta = 10$. Par contre, pour faire le parallèle avec les variations de $\alpha$, les algorithmes d'estimations (MLE ou GMH) semble toujours vouloir augmenter $\theta$ lorsque l'un ou l'autre augmente **(\@ref(fig:Theta100))**.
\
...
ggarrange(plot1, plot2, plot3, plot4, common.legend = TRUE)
```
Bien qu'assez faible, nous observons une dégradation de des estimations de GMH, notamment pour $\theta$ et $\alpha$. Les valeurs initiales données par l'expert peuvent donc entraîner de grandes variations par rapport aux vraies valeurs **(@ref(fig:MauvaisEleve2))**.
...
# **Conclusion**
Des variations sur au moins un des trois paramètres du processus ($\alpha,\ \beta \text{ et } \theta$) entraînent de nombreux changement dans les estimations via l'algorithme GMH. Ainsi, suivant les vraies valeurs choisies, la meilleure combinaison de lois a priori peut varier. Toutefois, la meilleure loi pour $\alpha$ reste toujours une log-normale, une loi gamma ayant montrée une baisse de la qualité d'estimation de manière quasiment systématique. En comparant aux estimations faites par maximum de vraisemblance, nous avons aussi constaté que l'estimation bayésienne n'était pas toujours la plus adapté suivant les cas. 
Nous pouvons aussi remarquer que ces estimations (MLE et GMH) semblent mieux adaptées pour retrouver la forme du processus plutôt que la valeur de ces paramètres. \
Comme attendu au niveau des paramètres relatif à l'expert, les estimations sont dégradées lorsque nous avons un mauvais expert, et le degré de confiance que nous lui accordons alors peut en partie corriger ses erreurs.
Pour aller plus loin, nous pourrions diversifié encore plus les cas d'expert, en ayant par exemple un bonne expertise pour $\beta$ mais une mauvaise pour les deux autres paramètres.
\
\


# **Utilisation de l'IA**
Nous avons utilisés ChatGPT-5 principalement pour corriger les erreurs de nos fonctions implémentées, pour nous aider dans le choix des lois a priori et pour nous aider à obtenir les visuels voulus.
