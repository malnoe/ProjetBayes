...
Afin d'analyser la pertinence de l'approche bayésienne face à un problème de ce type, nous avons séparé notre analyse en deux phases : i) la simulation du processus selon différentes valeurs pour $\alpha,\ \beta \text{ et } \theta$ afin d'observer leur impact respectif, et ii) l'étude de l'inférence bayésienne.
\
\
...
## *Contexte*
Nous disposons maintenant d'un échantillon d'inspection du niveau de dégradation $x = \{x_0,\dots,x_n\}$, obtenu à différents temps d'inspection $t = \{t_0,\dots,t_n\}$ et issus d'un processus gamma non-homogène de paramètres $\alpha,\ \beta \text{ et } \theta$.
Nous souhaitons alors mettre en place une inférence bayésienne multidimensionnelle afin d'estimer ses trois paramètres d'intérêts. Nous allons donc passer par les algorithmes de Gibbs et de Metropolis-Hasting en testant différentes combinaisons de loi a priori. Les trois paramètres étant positifs, et afin de se fixer un cadre d'étude pertinent \textcolor{red}{et assez court (à reformuler)}, nous restreignons le choix des lois théoriques de chaque paramètre à des lois à support positif :
  ...
  - Pour $\theta$, nous testerons une *loi inverse Gamma* $\mathcal{I\textit{nv-}G}(a,b)$.

Nous supposons aussi par la suite que l'expert nous indique toujours une estimation $\mu$ du paramètre à estimer (et non des paramètres de sa loi a priori) à laquelle nous associons une certaine confiance $\sigma$. Le but de cette étude étant plutôt de constater de l'impact du choix des valeurs et des lois de chaque paramètre, nous fixerons toujours $\mu$ et $\sigma$ dans un cadre où nous avons un bon expert et où nous lui faisons confiance, i.e. $\mu$ ne sera pas très loin de la vraie valeur du paramètre, et $\sigma$ sera assez "petit". \

Afin de juger de la pertinence de l'approche bayésienne, nous allons ses estimations à celles obtenues par l'approche du maximum de vraisemblance. Afin de pouvoir réaliser cette comparaison, nous calculons donc la vraisemblance du modèle.
...
En supposant que les paramètres d'expert $\mu_{\theta} \text{ et } \sigma_{\theta}$ correspondent à $\mu_{\theta} = \mathbb{E}[\mathcal{I\textit{nv}G}(a,b)] = \frac{b}{a-1}$ et que $\sigma_{\theta} = Var(\mathcal{I\textit{nv}G}(a,b)) = \frac{b^2}{(a-1)^2(a-2)}$, nous avons alors :
$$
\left\{\begin{array}{ll}
...
$$
ln(p(\alpha|y,\beta,\theta)) = (k-1)ln(\alpha)-\frac{\alpha}{s} + \sum_{i=1}^n(k_i(\alpha,\beta)-1)ln(y_i) - k_i(\alpha,\beta)ln(\theta) - ln(\Gamma(k_i(\alpha,\beta))) \text{ + constante}
$$
...
$$
\left\{\begin{array}{ll}
k = \frac{\mu_\alpha^2}{\sigma_\alpha} &.\\
s = \frac{\sigma_\alpha}{\mu_\alpha}
\end{array}\right.
$$
\
...
$$
\left\{\begin{array}{ll}
ln(p(\beta | y, \alpha, \theta)) = \sum_{i=1}^n (k_i(\alpha,\beta)-1)ln(y_i) - k_i(\alpha,\beta)ln(\theta) - ln(\Gamma(k_i(\alpha,\beta))) \text{ + cste} &\text{si } \beta\in[a,b]. \\
-\infty &\text{sinon}
\end{array}\right.
$$
...
La fonction `algo_GMH` implémente un algorithme MCMC de type Gibbs–Metropolis–Hastings (GMH) permettant d'obtenir K estimations bayésienne de chaque paramètre ($\alpha,\ \beta \text{ et } \theta$). Elle prend en entrée la liste d'arguments suivant :

  - `logpost_par` : fonction pour le calcul de log(posteriori($\tilde{\texttt{par}}$)).
  - `mu_par` : moyenne de l'expert pour le paramètre `par`.
  - `sigma_par` : confiance en l'expert pour le paramètre `par`.
  - `tau_par` : variation de la normale pour le paramètre `par`.
  - `par0` : initialisation du paramètre `par` pour chaîne de markov.
  - `data` : x_1, ..., x_n observations (à transformer en y_1, ..., y_n) les incréments.
  - `t_insp` : t_1,...,t_n les temps d'inspection.
...
Afin d'obtenir les MLE, nous cherchons à minimiser $-ln(Vraisemblance)$. Nous implémentons donc d'abord une fonction permettant de retourner cette objet (`neg_vraisemblance`), puis nous implémentons une fonction permettant d'exhiber les MLE obtenus (`mle`).
```{r}
# -Vraisemblance
...
Nous implémentons finalement une fonction nous permettant de visualiser les résultat obtenus simultanément (`vis_res`). Elle nous permettra notamment de comparer les vrais données aux estimations bayésiennes et par maximum de vraisemblance.
```{r}
