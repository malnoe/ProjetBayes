...
Afin de se fixer un cadre d'étude, nous restreignons les possibilités pour les lois théoriques de chaque paramètre :
  ...
  - $\alpha$ suivra soit une loi *lognormale*, soit une loi *Gamma*.
  - $\beta$ suivra soit une loi *lognormale*, soit une loi *Uniforme*.
...
algo_Gibbs_Metropolis_Hastings <- function(logpost_alpha,mu_alpha,sigma_alpha,tau_alpha,alpha0,
  ...
  # Calcul du a et b de l'inverse gamma pour simuler theta
  # à partir de moy = mu_theta et de var=sigma_theta
...
### Fonction calcul des estimateurs du maximum de vraisemblance par optimisation
...
```{r}
visualisation_resultat <- function(res_GMH,res_MLE,simulations_originales,subtitle=""){
  # Récupérer les informations de horizon, dt_insp, dt_sim de simulations originales
  horizon <- simulations_originales$grille_sim[length(simulations_originales$grille_sim)]
  dt_sim <- simulations_originales$grille_sim[2]-simulations_originales$grille_sim[1]
  dt_insp <- simulations_originales$grille_insp[2]-simulations_originales$grille_ins[1]
  
  # Extraire de res_GMH les alpha, beta et theta estimés
  alpha_GMH <- mean(res_GMH$res_alpha)
  beta_GMH <- mean(res_GMH$res_beta)
  theta_GMH <- mean(res_GMH$res_theta)
  # Faire simulations_GMH avec la fonction de simulation et les alpha,beta,theta_GMH
  simulations_GMH <- simulations(horizon,dt_sim,dt_insp,alpha=alpha_GMH,beta=beta_GMH,theta=theta_GMH)
  
  # Extraire de res_MLE les alpha, beta et theta estimés
  alpha_MLE <- res_MLE$alpha
  beta_MLE <- res_MLE$beta
  theta_MLE <- res_MLE$theta
  # Faire simulations_MLE avec la fonction de simulation et les alpha,beta,theta_MLE
  simulations_MLE <- simulations(horizon,dt_sim,dt_insp,alpha=alpha_MLE,beta=beta_MLE,theta=theta_MLE)
  
  # df_long avec grille_sim,X_sim,X_sim_GMH,X_sim_RLE
  df_long <- data.frame(grille_sim=simulations_originales$grille_sim,
                            X_sim=simulations_originales$X_sim,
                            X_sim_GMH=simulations_GMH$X_sim,
                            X_sim_MLE=simulations_MLE$X_sim)
  
  # ggplot avec les deux df_long avec 3 couleurs pour original, GMH et MLE.
  ggplot() +
    geom_line(
      aes(x = df_long$grille_sim, y = df_long$X_sim, 
          col = "Données originales")
      ) + 
    geom_line(
      aes(x = df_long$grille_sim, y = df_long$X_sim_GMH, 
          col = "Estimations bayésiennes")
      ) + 
    geom_line(
      aes(x = df_long$grille_sim, y = df_long$X_sim_MLE, 
          col = "Estimation maximum de vraissemblance")
      ) + 
    scale_color_manual(
      name = "A",
      values = c("Données originales" = "#dc0217",
                 "Estimations bayésiennes" = "#49a86c",
                 "Estimation maximum de vraissemblance" = "#428290" ),
    ) +
    labs(x = "t", y = expression(X[t]), subtitle = subtitle) +
    theme_light() +
    theme(
...
t_insp <- simulations1$grille_insp
K <- 10000
```
...
# 1. alpha~lognormale et beta~lognormale
res_GMH <- algo_Gibbs_Metropolis_Hastings(logpost_alpha_lognormale,mu_alpha,sigma_alpha,tau_alpha,alpha0,
                                           logpost_beta_lognormale,mu_beta,sigma_beta,tau_beta,beta0,
                                           mu_theta,sigma_theta,theta0,
                                           data,t_insp,K,burnin)
res_MLE <- mle(alpha0,beta0,theta0,data,t_insp)

plot1 <- visualisation_resultat(res_GMH,res_MLE,simulations1,subtitle="alpha~lognormale et beta~lognormale")

# 2. alpha~gamma et beta~lognormale
res_GMH <- algo_Gibbs_Metropolis_Hastings(logpost_alpha_gamma,mu_alpha,sigma_alpha,tau_alpha,alpha0,
                                           logpost_beta_lognormale,mu_beta,sigma_beta,tau_beta,beta0,
                                           mu_theta,sigma_theta,theta0,
                                           data,t_insp,K,burnin)
res_MLE <- mle(alpha0,beta0,theta0,data,t_insp)

plot2 <- visualisation_resultat(res_GMH,res_MLE,simulations1,subtitle="alpha~gamma et beta~lognormale")

# 3. alpha~lognormale et beta~unif
res_GMH <- algo_Gibbs_Metropolis_Hastings(logpost_alpha_lognormale,mu_alpha,sigma_alpha,tau_alpha,alpha0,
...
plot3 <- visualisation_resultat(res_GMH,res_MLE,simulations1,subtitle="alpha~lognormale et beta~unif")

# 4. alpha~gamma et beta~unif
res_GMH <- algo_Gibbs_Metropolis_Hastings(logpost_alpha_gamma,mu_alpha,sigma_alpha,tau_alpha,alpha0,
                                           logpost_beta_unif,mu_beta,sigma_beta,tau_beta,beta0,
                                           mu_theta,sigma_theta,theta0,
                                           data,t_insp,K,burnin)
...
plot4 <- visualisation_resultat(res_GMH,res_MLE,simulations1,subtitle="alpha~gamma et beta~unif")
